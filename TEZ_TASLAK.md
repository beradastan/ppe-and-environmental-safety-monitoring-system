...
Güvenlik Monitörü, birden fazla yazılım katmanının koordineli çalışmasına dayanan çok bileşenli bir sistem olarak tasarlanmıştır. Sistem mimari açıdan dört ana katmandan oluşmaktadır: tespit katmanı, uygulama katmanı, veri katmanı ve sunum katmanı.
...
Bir tespit hem alan oranı hem büyüme koşullarından en az birini karşıladığında ham yangın sinyali olarak kabul edilmektedir. Duman (`smoke`) tespitleri de aynı alan oranı ve büyüme filtrelerinden geçmektedir: her `smoke` bounding box'ının çerçeve alanına oranı ayrı bir geçmiş penceresinde (`_smoke_area_history`) takip edilmekte ve `fire_min_area_ratio` ile `fire_growth_factor` eşikleri uygulanmaktadır. Bu sayede küçük veya durağan duman tespitleri alarmı tetiklemeksizin bastırılmakta; yalnızca yeterince büyük ya da genişleyen duman bulutları ihlal sayılmaktadır. `use_fire` parametresi `false` olarak ayarlandığında hem yangın hem de duman tespiti tamamen devre dışı kalır.
...
- *Hangi PPE'ler tespit edilsin:* Baret, yelek, maske ve yangın + duman tespiti checkbox'larla etkinleştirilip devre dışı bırakılabilmektedir (`use_helmet`, `use_vest`, `use_mask`, `use_fire`). `use_fire` kapatıldığında hem yangın hem de duman tespiti devre dışı kalır.
- *Tespit eşikleri:* Her model için güven skoru eşiği (confidence threshold) kaydırıcı (slider) ile 0,05 - 0,95 aralığında ayarlanabilmektedir. Yangın + duman eşiği (`fire_conf`) hem yangın hem duman tespitine uygulanır.
...
- *Yangın + Duman filtresi:* Minimum alan oranı (`fire_min_area_ratio`) ve büyüme faktörü (`fire_growth_factor`) kaydırıcılarla yapılandırılabilmektedir; bu filtreler hem yangın hem duman tespitine eşit biçimde uygulanır. Büyüme penceresi (`fire_growth_window`) yalnızca `config.yaml` üzerinden ayarlanabilir.
...
Bu bölümde Güvenlik Monitörü'de kullanılan sekiz YOLO modelinin eğitim sonu test metrikleri sunulmaktadır. Tüm modeller Google Colab ortamında Tesla T4 GPU üzerinde eğitilmiş; test split'i eğitim ve doğrulama sürecinde hiçbir şekilde kullanılmamıştır.
...
| Özellik | Nath ve ark. (2020) [6] | Wu ve ark. (2019) [7] | Güvenlik Monitörü |
|---------|----------------------|---------------------|--------------|
...
| Metrik | Nath ve ark. 2020 [6] | Wu ve ark. 2019 [7] | Güvenlik Monitörü Crop | Güvenlik Monitörü Scene |
|--------|--------------------|------------------|-------------------|---------------------|
...
Bölüm 3.7'de gerçekleştirilen sistematik karşılaştırma, Güvenlik Monitörü'ün literatürdeki benchmark sistemlerine (Nath 2020 [6], Wu 2019 [7]) göre farklılaştığı boyutları ortaya koymaktadır. İncelenen 11 özellik açısından değerlendirildiğinde, Güvenlik Monitörü; çift mod PPE tespiti, kimlik kararlılığı (TrackReattacher), temporal oylama, kamera sağlık izleme ve yerel LLM entegrasyonu gibi işletimsel süreklilik ve raporlama gerektiren alanlarda belirgin biçimde farklılaşmaktadır. Nath 2020 ve Wu 2019 ile doğrudan metrik karşılaştırması farklı veri kümeleri nedeniyle metodolojik kısıtlara sahip olmakla birlikte, Güvenlik Monitörü'ün uçtan uca sistem bütünleşmesi (tespit → olay yönetimi → DB → LLM → web arayüzü) boyutunda mevcut çalışmalardan ayrıştığı görülmektedir.
...
**Proje Adı:** FABRİKA İŞ GÜVENLİĞİ İZLEME SİSTEMİ (Güvenlik Monitörü)  
**Tarih:** Mayıs 2026
